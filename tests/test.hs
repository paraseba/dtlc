...
boolTest =
  ...
  where
    program expr = booleans <> [expr]

defineEven :: NonEmpty ProgramStatement
defineEven =
    booleans
...
natsTest =
  ...
  where
    program expr = booleans <> naturals <> [expr]
...
vectorTests =
  ...
  where
    program expr = booleans <> defineEven <> vectors <> [expr]
...
assertReplResult program value typ =
    case res of
        ...
        _ -> assertFailure "No repl result"
  where
