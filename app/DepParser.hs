...
pi :: Parser ()
pi = syntax ["Pi", "forall", "Π", "∀"] <?> "Pi symbol"

arrow :: Parser ()
arrow = syntax ["->", "→"] <?> "Arrow symbol"

lambda :: Parser ()
lambda = syntax ["\\", "λ"] <?> "Lambda symbol"

assume :: Parser ()
assume = syntax ["assume"]

ofType :: Parser ()
ofType = syntax ["::"] <?> "Type of symbol"

typeKind :: Parser ()
typeKind = syntax ["*", "Type"] <?> "Type (*) kind"

lineEnd :: Parser ()
lineEnd  = void eol <|> eof

assumeParser :: Parser Statement
assumeParser = StAssume <$> (assume *>  parser)
  where
    parser =
...
kindAssumeParser = KindAssume <$> identifierParser <* ofType <* typeKind

typeAssumeParser :: Parser AssumeItem
typeAssumeParser = TypeAssume <$> (identifierParser <* ofType) <*> typeParser
